...
class Instance:
    ...
    def _detect_viewport(self,timeout_seconds:int=0.2):
        """
            ...
            5 : Loading Assets
        """
        print("Detecting Viewport")
        jy_main = api32.WindowControl(Name="剪映专业版",searchDepth=1,searchInterval=timeout_seconds)
        print(f"got jy_main {jy_main}")
        try:
            if jy_main.Exists(maxSearchSeconds=timeout_seconds)==False: return -1
        except : return -1
        print("jy_main exists")
    ...
    def _refresh_control(self):
        try:
            self.Window = api32.WindowControl(searchDepth=1,Name="剪映专业版")
            if "HomePageWebClassSupport" in self.Window.ClassName : self.Window = api32.WindowControl(Name="剪映专业版",searchDepth=1,foundIndex=self.Window.foundIndex+1)
