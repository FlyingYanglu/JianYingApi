...
# def _creat_exe(exepath: str):
#     try:
#         if not os.path.exists(exepath):
#             raise FileNotFoundError(f"Executable not found: {exepath}")
        
#         print(f"Starting executable: {exepath}")
#         process = subprocess.Popen(exepath, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
#         return process
...
def _creat_exe(exepath: str):
    process = multiprocessing.Process(target=run_exe, args=(exepath,))
    return process
